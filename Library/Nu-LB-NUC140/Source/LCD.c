...
void print_display_buffer()
{
    ...
    for (y = 0; y < (LCD_Ymax / 8); ++y) {
        for (x = 0; x < LCD_Xmax; ++x) {
			printf("%2x, ", DisplayBuffer[y*LCD_Xmax + x]);
        }
		printf("\n");
    }
		printf("\n\n");
...
void print_display_buffer_at(uint16_t x, uint16_t y)
{
    printf("%2x", DisplayBuffer[y * LCD_Xmax + x]);
	printf("\n");
}
void draw_Pixel_debug(int16_t x, int16_t y, uint16_t fgColor, uint16_t bgColor)
{	
	if (fgColor!=0) {
		// printf("origin: %2x\n", DisplayBuffer[x+y/8*LCD_Xmax]);
		DisplayBuffer[x+y/8*LCD_Xmax] |= (0x01<<(y%8));
		// printf("after: %2x\n", DisplayBuffer[x+y/8*LCD_Xmax]);
	}
	else 
